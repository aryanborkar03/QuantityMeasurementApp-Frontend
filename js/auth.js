...
function extractSessionUser(user) {
  return {
    id: user.id,
    name: user.name,
    email: user.email,
...
async function doLogin() {
  ...
  try {
    ...
    const user = users.find((item) => item.password === pw);
    ...
    setSession(extractSessionUser(user));
    window.location.href = 'pages/dashboard.html';
  } catch (error) {
    showError('loginError', 'Server is not running. Start json-server and try again.');
  }
}
...
async function doSignup() {
  ...
  if (!/^\d{10}$/.test(mobile)) {
    markInvalid('signupMobile');
    showError('signupError', 'Enter a valid 10-digit mobile number.');
    return;
  }

  try {
    const existingUsers = await fetchUsersByEmail(email);
    ...
    setSession(extractSessionUser(createdUser));
    window.location.href = 'pages/dashboard.html';
  } catch (error) {
    showError('signupError', 'Server is not running. Start json-server and try again.');
  }
}
